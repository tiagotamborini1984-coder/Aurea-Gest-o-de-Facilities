...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    let generated = 0, skipped = 0, errors = 0, backfill = 0
    ...
    for (const audit of audits || []) {
      ...
      if (existingChild) {
        console.log(`[process-recurring-audits] Child audit exists for "${audit.title}" period ${targetStr}, skipping`)
        skipped++; continue
      }
    ...
    const { data: allAudits } = await client.from('audits').select('id, title, client_id, sla_days').eq('status', 'Ativo')
    for (const a of allAudits || []) {
      const { data: pending } = await client.from('audit_executions')
        .select('id, task_id, plant_id, assignee_id').eq('audit_id', a.id)
        .in('status', ['Pendente', 'Rascunho']).is('task_id', null)
      for (const pe of pending || []) {
        const { typeId, statusId } = await getTaskConfig(client, a.client_id)
        if (!typeId || !statusId) continue
        const { data: admin } = await client.from('profiles').select('id')
          .eq('client_id', a.client_id).in('role', ['Administrador', 'Master']).limit(1)
        const reqId = admin?.[0]?.id || pe.assignee_id
        const due = a.sla_days != null
        ...
        const { data: task, error: te } = await client.from('tasks').insert({
          client_id: a.client_id, plant_id: pe.plant_id, type_id: typeId, status_id: statusId,
          requester_id: reqId, assignee_id: pe.assignee_id, task_number: 'GERANDO...',
          title: `Auditoria: ${a.title}`, due_date: due, status_updated_at: new Date().toISOString(),
          description: `Por favor, realize a auditoria "${a.title}" agendada.`, audit_id: a.id,
        }).select().single()
        if (te) { console.error(`[process-recurring-audits] Backfill failed: ${te.message}`); continue }
        await client.from('audit_executions').update({ task_id: task.id }).eq('id', pe.id)
        await client.from('task_timeline').insert({ task_id: task.id, user_id: reqId, content: `Tarefa gerada para "${a.title}".`, action_type: 'system' })
        backfill++
      }
    }

    console.log(`[process-recurring-audits] Done. Generated: ${generated}, Backfilled: ${backfill}, Skipped: ${skipped}, Errors: ${errors}`)
    return new Response(JSON.stringify({ success: true, generatedCount: generated, backfillCount: backfill, skippedCount: skipped, errorCount: errors }),
