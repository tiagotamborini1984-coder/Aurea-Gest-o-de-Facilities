...
function parseValue(s: string): { value: number; valid: boolean } {
  ...
  let n: string
  if (hasC && hasD) n = c.lastIndexOf(',') > c.lastIndexOf('.') ? c.replace(/\./g, '').replace(',', '.') : c.replace(/,/g, '')
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (isCsv) {
      ...
      const wb = XLSX.read(useSemi ? text.replace(/;/g, ',') : text, { type: 'string', codepage: 65001 })
    ...
    if (rows.length === 0) return jsonRes({ success: false, error: 'File is empty', inserted: 0, updated: 0, skipped: 0, total: 0, errors: [] })
    ...
    rows.forEach((row, i) => {
      ...
      if (!name) { errors.push(`Linha ${rn}: Coluna 'name' ausente ou vazia`); return }
      const valStr = getField(row, ['item_value', 'valor', 'valor_unitario', 'valor_unitário', 'Value'])
      let itemValue = 0
      if (valStr) {
      ...
      products.push({
        ...
        description: getField(row, ['description', 'descricao', 'descrição', 'Description']) || null,
        fs_code: getField(row, ['fs_code', 'codigo_fs', 'código_fs', 'FS']) || null,
        supply_code: getField(row, ['supply_code', 'codigo_supply', 'código_supply', 'Supply']) || null,
        unit_of_measure: getField(row, ['unit_of_measure', 'unidade', 'unidade_medida', 'Un']) || null,
    ...
    if (products.length === 0) return jsonRes({ success: false, error: 'No valid products', inserted: 0, updated: 0, skipped: 0, total: rows.length, errors })

    const { data: existingCats } = await userClient.from('inventory_categories').select('name').eq('client_id', clientId)
    const catSet = new Set((existingCats || []).map((c: any) => c.name.toLowerCase()))
    const newCats = new Set<string>()
    for (const p of products) { if (p.category && !catSet.has(p.category.toLowerCase())) newCats.add(p.category) }
    if (newCats.size > 0) {
      await userClient.from('inventory_categories').upsert(
        Array.from(newCats).map(name => ({ client_id: clientId, name })), { onConflict: 'client_id,name' }
      )
    }

    const { data: existing } = await userClient
      .from('inventory_products').select('id, name, supply_code, fs_code').eq('client_id', clientId)
    ...
    const strip = (p: any) => { const { row_number, ...rest } = p; return rest }
    ...
    for (let i = 0; i < bySupply.length; i += BATCH_SIZE) {
      ...
      const { data, error } = await userClient.from('inventory_products')
        .upsert(batch.map(strip), { onConflict: 'client_id,supply_code' }).select('id, supply_code')
      if (error) {
        for (const p of batch) {
          const { error: ie } = await userClient.from('inventory_products').upsert(strip(p), { onConflict: 'client_id,supply_code' })
          if (ie) errors.push(`Linha ${p.row_number}: ${ie.message}`)
          else if (exSupply.has(p.supply_code!)) updated++; else inserted++
        }
      } else if (data) {
        updated += existed; inserted += data.length - existed
    ...
    for (let i = 0; i < byFs.length; i += BATCH_SIZE) {
      ...
      const { data, error } = await userClient.from('inventory_products')
        .upsert(batch.map(strip), { onConflict: 'client_id,fs_code' }).select('id, fs_code')
      if (error) {
        for (const p of batch) {
          const { error: ie } = await userClient.from('inventory_products').upsert(strip(p), { onConflict: 'client_id,fs_code' })
          if (ie) errors.push(`Linha ${p.row_number}: ${ie.message}`)
          else if (exFs.has(p.fs_code!)) updated++; else inserted++
        }
      } else if (data) {
        updated += existed; inserted += data.length - existed
    ...
    const toInsFiltered = toInsert.filter(p => {
      if (exNames.has(p.name.toLowerCase())) { skipped++; return false }
      return true
    })
    for (let i = 0; i < toInsFiltered.length; i += BATCH_SIZE) {
      const batch = toInsFiltered.slice(i, i + BATCH_SIZE)
      const { data, error } = await userClient.from('inventory_products').insert(batch.map(strip)).select('id')
      if (error) {
        for (const p of batch) {
          const { error: ie } = await userClient.from('inventory_products').insert(strip(p))
          if (ie) errors.push(`Linha ${p.row_number}: ${ie.message}`); else inserted++
        }
    ...
    return jsonRes({ success: false, error: error.message, inserted: 0, updated: 0, skipped: 0, total: 0, errors: [] }, 400)
  }
})
