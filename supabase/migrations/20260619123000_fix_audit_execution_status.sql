DO $DO$
BEGIN
  -- Ensure that any audit execution that has answers but is not in a terminal state is marked as Finalizado
  UPDATE public.audit_executions ae
  SET status = 'Finalizado'
  WHERE lower(status) NOT IN ('finalizado', 'finalizada', 'concluido', 'concluído', 'concluida', 'concluída', 'realizado', 'realizada', 'finished', 'completed')
    AND EXISTS (
      SELECT 1 FROM public.audit_execution_answers aea WHERE aea.execution_id = ae.id
    );

  -- Update tasks linked to those executions to be marked as finalized (if terminal status exists)
  UPDATE public.tasks t
  SET status_id = (
    SELECT id FROM public.task_statuses 
    WHERE is_terminal = true AND client_id = t.client_id 
    ORDER BY created_at ASC LIMIT 1
  )
  WHERE t.id IN (
    SELECT ae.task_id 
    FROM public.audit_executions ae 
    WHERE ae.status = 'Finalizado' AND ae.task_id IS NOT NULL
  )
  AND t.status_id NOT IN (
    SELECT id FROM public.task_statuses WHERE is_terminal = true AND client_id = t.client_id
  );

  -- Fix RLS for audit_executions (authenticated select)
  ...
  -- Fix RLS for audit_execution_answers (authenticated select)
