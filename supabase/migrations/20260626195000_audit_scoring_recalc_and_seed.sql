...
DECLARE
  ...
  new_user_id uuid;
BEGIN
  -- 1. Create or ensure the test user exists
  IF NOT EXISTS (SELECT 1 FROM auth.users WHERE email = 'lptamborini@hotmail.com') THEN
    ...
    VALUES (new_user_id, 'lptamborini@hotmail.com', 'Admin Tester', 'Master')
    ON CONFLICT (id) DO NOTHING;
  END IF;

  -- 2. Recalculate historical data for finished audits with missing or 0 scores
  FOR v_row IN 
...
-- 3. Update submit_audit_execution to handle nested `value` defensively and be atomic
