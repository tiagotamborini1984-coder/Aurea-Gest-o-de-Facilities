        ...
        IF v_type_id IS NOT NULL AND v_status_id IS NOT NULL THEN
          ...
          BEGIN
            INSERT INTO public.tasks (
              ...
              v_target_date, CURRENT_TIMESTAMP, v_audit.id
            ) RETURNING id INTO v_task_id;

            INSERT INTO public.audit_executions (
              audit_id, task_id, assignee_id, plant_id, status
            )
            VALUES (NEW.audit_id, v_task_id, NEW.assignee_id, NEW.plant_id, 'Pendente');
          ...
          EXCEPTION WHEN OTHERS THEN
            ...
            VALUES (
              ...
              'Recurrence task creation failed for audit "' || COALESCE(v_audit.title, NEW.audit_id::text) ||
              '": ' || SQLERRM
            );
          END;
        ELSE
          ...
          VALUES (
            ...
            COALESCE(v_requester_id, NEW.assignee_id),
            'task_creation_failed',
            'Missing prerequisites for recurrence of audit "' || COALESCE(v_audit.title, NEW.audit_id::text) || '"'
          );
        END IF;
      END IF;
    END IF;
