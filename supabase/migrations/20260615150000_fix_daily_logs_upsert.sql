...
CREATE POLICY "daily_logs_insert" ON public.daily_logs
  ...
  WITH CHECK (
    (client_id = public.get_user_client_id() OR EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'Master'))
    AND (
      EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role IN ('Administrador', 'Master'))
