-- ============================================================================
-- Migration: Definitive strict active-only employee filtering for attendance
-- Ensures get_attendance_employees ONLY returns status = 'Ativo' employees
-- Deactivates specific dismissed collaborators at the Sorriso plant
-- Fully idempotent: safe to run multiple times
-- ============================================================================

-- ============================================================================
-- 0. Fix trigger_audit_daily_logs to handle NULL auth.uid() during migrations
-- ============================================================================
...
--    Strictly returns only active employees with their daily log status
-- ============================================================================
...
--    Strictly returns only active employees — no exceptions
-- ============================================================================
CREATE OR REPLACE FUNCTION public.get_attendance_employees(
  p_plant_id uuid,
...
-- ============================================================================
-- 5. Deactivate specific dismissed collaborators at the Sorriso plant (idempotent)
--    Only updates rows that are not already 'Inativo' to minimize writes
-- ============================================================================
...
-- ============================================================================
-- 6. Also deactivate matching org_collaborators at Sorriso plant (idempotent)
-- ============================================================================
...
-- ============================================================================
-- 7. Clean up orphaned unpublished daily_logs for inactive employees at Sorriso
--    Preserves published historical records
-- ============================================================================
...
-- ============================================================================
-- 8. Ensure RLS SELECT policy exists on employees table for authenticated users
--    The policy enforces tenant isolation but does NOT filter by status —
--    status filtering is the responsibility of the RPC and application layer.
--    This is the correct separation of concerns: RLS = data access control,
--    application logic = business rules (active vs inactive).
-- ============================================================================
...
CREATE POLICY "employees_select" ON public.employees
  FOR SELECT TO authenticated USING (
    EXISTS (
      ...
      WHERE p.id = auth.uid()
      AND (p.client_id = employees.client_id OR lower(p.role) = 'master')
    )
  );

-- ============================================================================
-- 9. Ensure RLS policies for daily_logs are correct
-- ============================================================================
DROP POLICY IF EXISTS "daily_logs_select" ON public.daily_logs;
CREATE POLICY "daily_logs_select" ON public.daily_logs
  FOR SELECT TO authenticated USING (
    (lower(get_user_role()) = 'master' OR client_id = get_user_client_id())
