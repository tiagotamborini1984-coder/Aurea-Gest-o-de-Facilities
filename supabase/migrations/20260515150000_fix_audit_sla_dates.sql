...
BEGIN
  -- Fix tasks for recurring audits that have due_date in the past but are pending
  FOR v_task IN (
    SELECT t.id, t.created_at, t.due_date, e.audit_id
    FROM public.tasks t
    JOIN public.audit_executions e ON e.task_id = t.id
    WHERE t.status_id IN (SELECT id FROM public.task_statuses WHERE is_terminal = false)
      AND e.status = 'Pendente'
      AND t.title LIKE 'Auditoria:%'
      AND t.due_date < t.created_at
  ) LOOP
    SELECT * INTO v_audit FROM public.audits WHERE id = v_task.audit_id;
    IF FOUND AND v_audit.frequency != 'Única' THEN
      v_next_date := (v_task.created_at AT TIME ZONE 'UTC')::DATE;
      
      -- Advance v_next_date if it's in the past relative to today
      ...
      v_new_due_date := v_next_date + TIME '23:59:59.999';
...
BEGIN
  IF NEW.status = 'Finalizado' AND OLD.status != 'Finalizado' THEN
    ...
    IF v_audit.frequency != 'Única' THEN
      -- Calculate next date based on realization_date or created_at
      ...
      -- Check if we need to advance to future if the calculated is in the past
      WHILE v_next_date < CURRENT_DATE LOOP
        IF v_audit.frequency = 'Diária' THEN
          v_next_date := v_next_date + INTERVAL '1 day';
        ELSIF v_audit.frequency = 'Semanal' THEN
      ...
      -- Check if there's already a pending execution (ignoring assignee_id to prevent duplicates on assignee change)
      IF NOT EXISTS (
