...
//     USING: (((client_id = get_user_client_id()) OR (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = 'Master'::text))))) AND ((EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['Administrador'::text, 'Master'::text]))))) OR (plant_id IN ( SELECT user_plants.plant_id    FROM user_plants   WHERE (user_plants.user_id = auth.uid())))))
//   Policy "daily_logs_insert" (INSERT, PERMISSIVE) roles={authenticated}
//     WITH CHECK: (((client_id = get_user_client_id()) OR (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = 'Master'::text))))) AND ((EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['Administrador'::text, 'Master'::text]))))) OR (plant_id IN ( SELECT user_plants.plant_id    FROM user_plants   WHERE (user_plants.user_id = auth.uid())))))
//   Policy "daily_logs_select" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: ((client_id = get_user_client_id()) OR (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = 'Master'::text)))))
