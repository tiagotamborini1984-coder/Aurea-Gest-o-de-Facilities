...
//     USING: (EXISTS ( SELECT 1    FROM profiles p   WHERE ((p.id = auth.uid()) AND ((p.client_id = daily_logs.client_id) OR (p.role = 'Master'::text)) AND ((p.role = ANY (ARRAY['Administrador'::text, 'Master'::text, 'Gestor'::text])) OR ((p.authorized_plants IS NOT NULL) AND ((p.authorized_plants @> to_jsonb(daily_logs.plant_id)) OR (p.authorized_plants @> to_jsonb((daily_logs.plant_id)::text))))))))
