...
//   client_id: uuid (not null)
//   name: text (not null)
//   category: text (not null, default: 'Reativo'::text)
//   created_at: timestamp with time zone (not null, default: now())
...
//     WITH CHECK: ((get_user_role() = 'Master'::text) OR (client_id = get_user_client_id()))
// Table: locker_occupations
//   Policy "tenant_isolation_locker_occupations" (ALL, PERMISSIVE) roles={authenticated}
//     USING: ((get_user_role() = 'Master'::text) OR (client_id = get_user_client_id()))
//     WITH CHECK: ((get_user_role() = 'Master'::text) OR (client_id = get_user_client_id()))
// Table: lockers
//   Policy "tenant_isolation_lockers" (ALL, PERMISSIVE) roles={authenticated}
//     USING: ((get_user_role() = 'Master'::text) OR (client_id = get_user_client_id()))
//     WITH CHECK: ((get_user_role() = 'Master'::text) OR (client_id = get_user_client_id()))
// Table: maintenance_assets
...
//   CREATE OR REPLACE FUNCTION public.submit_maintenance_ticket(p_client_id uuid, p_plant_id uuid, p_location_id uuid, p_sublocation_id uuid, p_asset_id uuid, p_requester_name text, p_requester_email text, p_description text, p_photos jsonb)
//    RETURNS jsonb
//    LANGUAGE plpgsql
//    SECURITY DEFINER
//   AS $function$
